-- ============================================================
-- FIX: Drop old guest_records table (wrong column names)
--      and recreate with correct snake_case column names
-- 
-- Run this in Supabase Dashboard → SQL Editor
-- URL: https://supabase.com/dashboard/project/wwxbzprkagdhuitkpyqs/sql
-- ============================================================

-- Step 1: Drop the old table (it has column names with spaces)
DROP TABLE IF EXISTS guest_records CASCADE;

-- Step 2: Create the correct table with proper snake_case columns
...
-- Step 5: Enable Row Level Security (RLS) - allows service role full access
ALTER TABLE guest_records ENABLE ROW LEVEL SECURITY;

-- Allow service_role to do everything (backend uses service key)
CREATE POLICY "Service role full access" ON guest_records
  FOR ALL
  TO service_role
  USING (true)
  WITH CHECK (true);
...
WHERE table_name = 'guest_records' AND table_schema = 'public'
